...
export function AppHeader() {
  ...
  // Filtrar itens de menu baseado nas permissoes do usuario
  const navItems = ALL_NAV_ITEMS.filter((item) => {
        if (isMaster) return true; // master ve tudo
                                            if (item.path === "/financeiro") return hasPermission("financeiro");
        if (item.path === "/user-management") return false; // apenas master
                                            return true; // Dashboard, Processos, Audiencias sao visiveis para todos
  });
        ...
        <header className="w-full bg-slate-900 text-white sticky top-0 z-50">
              <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-2">
                                ...
                                <span className="font-bold text-lg">Facilita ADV</span>span>
                      </div>div>
                                        ...
                                        <div className="flex items-center justify-between">
                                                      <div
                                                        ...
                                                        {user?.avatar_url ? (
                                                                      ...
                                                                      <div className="flex flex-col">
                                                                                        <span className="text-sm text-white">{user?.nome || user?.email?.split("@")[0] || "Usuario"}</span>span>
                                                                                        <span className="text-xs text-gray-400">{getFuncaoLabel(user?.funcao)}</span>span>
                                                                      </div>div>
                                                      </div>div>
                                                      <Button
                                                                        variant="outline"
